...
默认情况下，插件不会读取机构页面中的密码，也不会复制或导出 Cookie。用户可在设置中显式勾选“自动登录”，输入账号和密码后点击“安全保存凭据”；凭据由 Zotero Password Manager 受保护地保存，不写入插件偏好设置、日志或配置导出。自动填写只会发生在与“登录 URL（SSO/CAS 页面）”完全同源的 HTTPS 页面，因此应填写实际的统一认证/CAS 登录地址，而不仅是 WebVPN 网关地址。
